...
###############################################################################
###                                                                         ###
###              Data preparation script for 2025 WIDA GA data              ###
###                                                                         ###
###############################################################################
...
#'
#' For the 2025 Georgia WIDA ACCESS for ELLs data preparation and cleaning, we
...
###   Read in 2025 base data
WIDA_GA_Data_LONG_2025 <-
    fread(file = "Data/Base_Files/ACCESS_2025_DataForSGP_cleaned-prepped.csv",
...
##    Subset data
WIDA_GA_Data_LONG_2025 <- subset(WIDA_GA_Data_LONG_2025, select = wida.ga.names)
...
##    Check tables with Elena's ('NCIEA_Check_Data_Import_2025.docx')
# table(WIDA_GA_Data_LONG_2025[, VALID_CASE])
# WIDA_GA_Data_LONG_2025[, VALID_CASE, GRADE] |>
# # WIDA_GA_Data_LONG_2025[, VALID_CASE, ACHIEVEMENT_LEVEL_ORIGINAL] |>
...
#     table(WIDA_GA_Data_LONG_2025[ID_CHECK < 10, ID])
][, ID_CHECK := nchar(ID)
][ID_CHECK < 10,
    VALID_CASE := "INVALID_CASE" # 1 invalid case added in 2025 (1,689 total invalids)
][, ID_CHECK := NULL
]
...
WIDA_GA_Data_LONG_2025[
    ...
    VALID_CASE := "INVALID_CASE" # 0 invalid cases ADDED in 2025 (52 already invalid)
]
...
sum(dupl) # 39 dups - All invalid already invalid
table(WIDA_GA_Data_LONG_2025[, VALID_CASE] == "INVALID_CASE", dupl)
WIDA_GA_Data_LONG_2025[
    c(which(dupl), which(dupl) - 1),
][VALID_CASE == "VALID_CASE"] |> setkeyv(key(WIDA_GA_Data_LONG_2025)) |> print() # 4 students (most switched schools)
WIDA_GA_Data_LONG_2025[
...
sum(dupl)  #  46 dups - All invalid already invalid
table(WIDA_GA_Data_LONG_2025[, VALID_CASE] == "INVALID_CASE", dupl)
WIDA_GA_Data_LONG_2025[
    which(dupl) - 1, # Invalidate lowest score (-1 index)
    VALID_CASE := "INVALID_CASE"
]
setkey(WIDA_GA_Data_LONG_2025, VALID_CASE, CONTENT_AREA, YEAR, ID)


### Save data
save(WIDA_GA_Data_LONG_2025, file = "Data/WIDA_GA_Data_LONG_2025.Rdata")
