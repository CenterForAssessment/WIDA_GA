...
###############################################################################
###                                                                         ###
###              Data preparation script for 2024 WIDA GA data              ###
###                                                                         ###
###############################################################################
...
#'
#' For the 2024 Georgia WIDA ACCESS for ELLs data preparation and cleaning, we
...
###   Read in 2024 base data
WIDA_GA_Data_LONG_2024 <-
    fread(file = "Data/Base_Files/ACCESS_2024_DataForSGP_cleaned-prepped.csv",
...
##    Subset data
WIDA_GA_Data_LONG_2024 <- subset(WIDA_GA_Data_LONG_2024, select = wida.ga.names)
...
##    Check tables with Adrienne's ('NCIEA_Check_Data_Import_2024.docx')
# table(WIDA_GA_Data_LONG_2024[, VALID_CASE])
# # WIDA_GA_Data_LONG_2024[, VALID_CASE, sort(as.numeric(GRADE))] |>
# WIDA_GA_Data_LONG_2024[, VALID_CASE, ACHIEVEMENT_LEVEL_ORIGINAL] |>
...
#     table(WIDA_GA_Data_LONG_2024[ID_CHECK < 10, ID])
][, ID_CHECK := nchar(ID)
][ID_CHECK < 10,
    VALID_CASE := "INVALID_CASE" # 7 invalid cases added in 2024 (1,651 total invalids)
][, ID_CHECK := NULL
]
...
WIDA_GA_Data_LONG_2024[
    ...
    VALID_CASE := "INVALID_CASE" # 0 invalid cases added in 2024
]
...
sum(dupl) # 62 dups - All invalid already invalid
table(WIDA_GA_Data_LONG_2024[, VALID_CASE] == "INVALID_CASE", dupl)
WIDA_GA_Data_LONG_2024[
    c(which(dupl), which(dupl) - 1),
][VALID_CASE == "VALID_CASE"] |> setkeyv(key(WIDA_GA_Data_LONG_2024)) |> print() # 4 students (most switched schools)
WIDA_GA_Data_LONG_2024[
...
sum(dupl)  #  71 dups - All invalid already invalid
table(WIDA_GA_Data_LONG_2024[, VALID_CASE] == "INVALID_CASE", dupl)
WIDA_GA_Data_LONG_2024[
    which(dupl) - 1, # Invalidate lowest score (-1 index)
    VALID_CASE := "INVALID_CASE"
]
setkey(WIDA_GA_Data_LONG_2024, VALID_CASE, CONTENT_AREA, YEAR, ID)


### Save data
save(WIDA_GA_Data_LONG_2024, file = "Data/WIDA_GA_Data_LONG_2024.Rdata")
